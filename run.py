...
@contextmanager
def run_in_dev():
    from weppy_mvc_demo.dev_utils import remove_admin, remove_user
    try:
        yield
    finally:
        remove_admin()
...
if __name__ == "__main__":
    arg_parser = ArgumentParser(description="Weppy Mvc Demo running utility.")
    arg_parser.add_argument('-d', '--dev', help="Setup add dev users and enable verbose logging",
                            action='store_true')
    args = arg_parser.parse_args()
    if args.dev:
        from weppy_mvc_demo.dev_utils import setup_admin, setup_user
